...
class ResNet_for_Imagenet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 ...
                 group_number: int = 72,
                 left_shift_bit: int = 3):
        super(ResNet_for_Imagenet, self).__init__()
        ...
        # 是否考虑FP8量化及硬件特性
        if fp_on == 1:
            self.conv1 = Conv2d_fp8(3, self.inplanes, kernel_size=3, stride=2, padding=1, bias=False)
        elif fp_on == 2:
            self.conv1 = Conv2d_fp8_hw(3, self.inplanes, kernel_size=3, stride=2, padding=1, bias=False,
                                       quant_type=quant_type, group_number=group_number, left_shift_bit=left_shift_bit)
        elif (qn_on):
            self.conv1 = Conv2d_quant(qn_on=qn_on, in_channels=3, out_channels=self.inplanes,kernel_size=3,stride=2, padding=1, bias=False,
                                      weight_bit=weight_bit, output_bit=output_bit, isint=isint, clamp_std=clamp_std)
        else:
...
def ResNet18_for_Imagenet(**kwargs):
    return ResNet_for_Imagenet(BasicBlock, [2, 2, 2, 2], **kwargs)
